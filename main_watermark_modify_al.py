...
def main():
    ...
    # assert DATA_DIR
    DATA_DIR = '/root/autodl-tmp/other/OpenImages/validation' 
    ...
    if not opt.debug:
        try:
            ...
            if opt.test == '':
                ...
                experiment_dir = cur_time + "_" + str(opt.imageSize)+ "_"+ str(opt.num_secret) + "_"+ str(opt.num_training)+ "_" + \
    ...
    if opt.test == '':
        # tensorboardX writer
        if not opt.debug:
            writer = SummaryWriter(log_dir='training/' + experiment_dir)
