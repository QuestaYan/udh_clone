...
parser.add_argument('--test', default='autodl-container-3f8d48b962-24e2d5b1_2024-11-27_H11-21-48_256_1_1_32_1_instance_l2_0.75_1colorIn1color_', help='checkpoint folder')#测试，输入路径名称
...
def main():
    ...
    if opt.checkpoint != "":
        if opt.checkpoint_diff != "":
            ...
            Hnet.load_state_dict(checkpoint['H_state_dict'])#Hnet.load_state_dict(checkpoint_diff['H_state_dict'])
            Rnet.load_state_dict(checkpoint['R_state_dict'])            
...
def train(train_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, ((secret_img, secret_target), (cover_img, cover_target)) in enumerate(train_loader, 0):
        ...
        err_sum = errH + betaerrR_secret#水印嵌入器和提取器两个loss相加
        optimizer.zero_grad()
        err_sum.backward()
        optimizer.step()
