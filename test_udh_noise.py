#在main_modify.py基础上增加udh模型噪声测试功能,only for udh
# encoding: utf-8
import csv
import argparse
import os
...
from skimage.metrics import peak_signal_noise_ratio as PSNR # from skimage.measure import compare_ssim as SSIM, compare_psnr as PSNR
import sys   #导入sys模块
sys.path.append("/mnt/e/run_code/watermark/01-2021-MM-MBRS/")
...
# parser.add_argument('--epochs', type=int, default=5,#65
...
# parser.add_argument('--bs_secret', type=int, default=16, help='batch size for ')
...
# parser.add_argument('--noise_type', default='identity', help='Noise type (identity | crop | cropout | dropout | gaussian | jpeg_50 | jpeg_85)')
...
#     else:
#         opt.checkpoint = "./模型文件/autoencoder对比结果/" + opt.test + "/checkPoints/" + "checkpoint.pth.tar"
#         if opt.test_diff != '':
...
#     if opt.norm == 'none':
#         norm_layer = None
#     if opt.cover_dependent:#(依赖，DDH)
#         Hnet = UnetGenerator(input_nc=opt.channel_secret*opt.num_secret+opt.channel_cover*opt.num_cover, output_nc=opt.channel_cover*opt.num_cover, num_downs=num_downs, norm_layer=norm_layer, output_function=nn.Sigmoid).cuda()
#     else:#（独立，UDH）
#         Hnet = UnetGenerator(input_nc=opt.channel_secret*opt.num_secret, output_nc=opt.channel_cover*opt.num_cover, num_downs=num_downs, norm_layer=norm_layer, output_function=nn.Tanh).cuda()
#     Rnet = RevealNet(input_nc=opt.channel_cover*opt.num_cover, output_nc=opt.channel_secret*opt.num_secret, nhf=64, norm_layer=norm_layer, output_function=nn.Sigmoid).cuda()
...
#     HnetD = UnetGenerator(input_nc=opt.channel_secret*opt.num_secret+opt.channel_cover*opt.num_cover, output_nc=opt.channel_cover*opt.num_cover, num_downs=num_downs, norm_layer=norm_layer, output_function=nn.Sigmoid).cuda()
#     RnetD = RevealNet(input_nc=opt.channel_cover*opt.num_cover, output_nc=opt.channel_secret*opt.num_secret, nhf=64, norm_layer=norm_layer, output_function=nn.Sigmoid).cuda()
#     HnetD.apply(weights_init)
#     RnetD.apply(weights_init)
    
#     noiser_identity = Identity().cuda()
#     noiser_dropout = Dropout([0.3, 0.3]).cuda()
#     noiser_jpeg50 = JpegCompression(quality=50).cuda()
#     noiser_jpeg85 = JpegCompression(quality=85).cuda()
...
# def analysis(val_loader, epoch, Hnet, Rnet, HnetD, RnetD, criterion):###test，测试
#     print(
#         "#################################################### analysis begin ########################################################")
...
#         ssim = np.zeros(N)
#         for i in range(N):
#             ssim[i] = SSIM(cover_img_numpy[i], container_img_numpy[i], channel_axis=-1,data_range=1.0)      # 颜色通道在最后一维#SSIM(cover_img_numpy[i], container_img_numpy[i], multichannel=True)
...
#         ssim = np.zeros(N)
#         for i in range(N):
#             ssim[i] = SSIM(cover_img_numpy[i], container_img_numpy[i], channel_axis=-1,data_range=1.0)#ssim[i] = SSIM(secret_img_numpy[i], rev_secret_numpy[i], multichannel=True)
...
#         ####################################### Cover Agnostic S' 固有的secret恢复重建能力#######################################
#         cover_imgv, container_img, secret_imgv_nh, rev_secret_img, errH, errR, diffH, diffR \
...
#         # ssim = np.zeros(N)
#         # for i in range(N):
#         #     ssim[i] = SSIM(cover_img_numpy[i], container_img_numpy[i], channel_axis=-1,data_range=1.0)#ssim[i] = SSIM(cover_img_numpy[i], container_img_numpy[i], multichannel=True)
#         # print("Avg. SSIM C:", ssim.mean().item())
